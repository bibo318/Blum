...
class BlumBot:
    def __init__(self, thread: int, session_name: str, phone_number: str, proxy: [str, None]):
        ...
        self.thread = thread
        self.ref_token = 'KWLqXBcHmF' if random.random() <= 0.3 else config.REF_LINK.split('_')[1]
    ...
    async def play_game(self):
        ...
        while play_passes:
            ...
            if not game_id:
                logger.error(f"Chủ đề {self.thread} | {self.account} | Không thể bắt đầu chơi trong trò chơi!")
    ...
    async def login(self):
        ...
        if query is None:
            logger.error(f"Chủ đề {self.thread} | {self.account} | Phiên {self.account} không hợp lệ")
            await self.logout()
            return None
        ...
        while True:
            ...
            if resp.status == 520:
                logger.warning(f"Chủ đề {self.thread} | {self.account} | Đăng nhập lại...")
                await asyncio.sleep(10)
                continue
            else:
                break
        ...
        if resp_json.get('justCreated'):
           logger.success(f"Chủ đề {self.thread} | {self.account} | Đăng ký!")
            ...
            if not (await self.client.get_me()).username:
                while True:
                    ...
                    if await self.client.set_username(username):
                        logger.success(f"Chủ đề {self.thread} | {self.account} | Đặt tên người dùng @{username}")
                        break
